...
**UI 개선 (모바일 디자인 대폭 개선)**
- 모든 테두리 선 제거 (카드, 버튼, 탑바, 하단 네비, 필터 등) — 깔끔한 디자인
- 북마크 아이콘: 빈 별(☆) → 채운 회색별(★)으로 변경
- 스와이프 가이드 좌측 정렬 + 우측에 원형 단어 추가(+) 버튼 배치
- 상단 "추가" 버튼 제거 (스와이프 가이드 행으로 이동)
- 카드 번호 숨김 처리
- 범위 선택 바(전체 1-2035) 숨김
- 검색창 크기 축소 및 테두리 제거
- 통계 바: 폰트 축소, 중앙 정렬, 균일 간격
- 영단어 폰트 확대 (1.2rem), 뜻 폰트 통일 (0.82rem)
- 발음 정지 아이콘(◼) 크기 확대
...
- 모바일 버튼 터치 안 되는 문제 해결 (ontouchstart stopPropagation으로 스와이프 핸들러 차단)
- 스와이프 핸들러가 북마크/재생 버튼 터치를 가로채는 문제 수정
- 카카오톡 인앱 브라우저 등 Web Speech API 미지원 환경 대응
- 캐시 방지: 메타태그 추가 + index.html 리다이렉트 시 타임스탬프 파라미터 적용
